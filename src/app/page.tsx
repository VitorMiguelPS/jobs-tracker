  ...
  if (jobs.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        ...
        <h1 className="text-2xl font-bold text-gray-900 mb-2">Comece sua jornada</h1>
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-500 mt-1">Resumo das suas candidaturas</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {byStatus.map((group) => (
          <div key={group.label} className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between mb-3">
              <h3 className={`text-sm font-semibold ${group.color}`}>{group.label}</h3>
              <span className="text-2xl font-bold text-gray-900">{group.items.length}</span>
            </div>
            {group.items.length > 0 ? (
              ...
              <p className="text-sm text-gray-400 text-center py-2">Nenhuma</p>
            )}
          </div>
        ))}
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-gray-900">Candidaturas recentes</h2>
          <Link
            href="/vagas"
